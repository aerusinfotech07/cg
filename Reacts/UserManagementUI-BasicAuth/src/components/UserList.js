...
const UserList = () => {
  ...
  return (
    <div>
      <div className="page-header"><h1>User Management (Basic Auth)</h1></div>
      {error && (
      ...
      <div className="table-container">
        ...
        {users.length === 0 ? (
          ...
          <table className="table">
            <thead>
              <tr>
                <th>ID</th><th>Name</th><th>Username</th><th>Email</th><th>Phone</th><th>Website</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {users.map((user) => (
                <tr key={user.id}>
                  ...
                  <td>{user.website}</td>
                  <td>
                    <button className="btn btn-info btn-sm" onClick={() => navigate(`/view/${user.id}`)}>View</button>
                    <button className="btn btn-warning btn-sm" onClick={() => navigate(`/edit/${user.id}`)}>Edit</button>
                    {deleteConfirm === user.id ? (
                      ...
                      <button className="btn btn-danger btn-sm" onClick={() => setDeleteConfirm(user.id)}>Delete</button>
                    )}
                  </td>
                </tr>
              ))}
