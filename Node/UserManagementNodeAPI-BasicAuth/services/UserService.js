...
class UserService {
    ...
    async updateUser(id, userData) {
        ...
        try {
            ...
            if (userData.username && userData.username !== existingUser.username && (await this.existsByUsername(userData.username))) {
                throw new Error('Username already exists: ' + userData.username);
            }
            if (userData.email && userData.email !== existingUser.email && (await this.existsByEmail(userData.email))) {
                throw new Error('Email already exists: ' + userData.email);
            }
            if (userData.address && userData.address.geo) {
            ...
            if (userData.address) {
                if (existingUser.address) {
                    ...
                    await connection.execute(
                        ...
                        [av.street, av.suite, av.city, av.zipcode, existingUser.address.id]
                    );
                } else {
                    let geoId = null;
            ...
            if (userData.company) {
                if (existingUser.company) {
                    ...
                    await connection.execute(
                        ...
                        [cv.name, cv.catch_phrase, cv.bs, existingUser.company.id]
                    );
                } else {
                    const company = new Company(userData.company);
                    const cv = company.toDbValues();
            ...
            await connection.execute(
                ...
                [uv.name || existingUser.name, uv.username || existingUser.username, uv.email || existingUser.email,
                 uv.phone !== undefined ? uv.phone : existingUser.phone, uv.website !== undefined ? uv.website : existingUser.website, id]
            );
            await connection.commit();
